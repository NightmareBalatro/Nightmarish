# Changelog

## V 1.0.0
  * Nerfs jokers by making them word than in the vanilla gamne. Less mult, chips, xmult, worse odds, higher rarity and or higher cost

## V 1.1.0
  * Fixes legendary rarity for modified jokers. Jokers with rarity = 4 no longer appear in shop or buffon packs

## V 1.2.0
  * Switches to manifest.json format for smods
  * Adds mod logo
  * Split files between logic & joker nerf tables
  * Adds Raised Fist nerf, Raised Fist now only gives 1x the mult of the lowest card in hand

## V 1.3.0n, Nightmare's Update
  * Nerfs most Jokers AGAIN, ontop of pervious nerfs
  * Changed authors to asciidisco, and Nightmare
  * Changed description to be grammatically correct